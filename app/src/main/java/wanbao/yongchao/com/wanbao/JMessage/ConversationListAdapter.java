...
public class ConversationListAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, final ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(mContext).inflate(jiguang.chat.R.layout.item_conv_list, null);
        }
        final ImageView headIcon = ViewHolder.get(convertView, jiguang.chat.R.id.msg_item_head_icon);
        TextView convName = ViewHolder.get(convertView, jiguang.chat.R.id.conv_item_name);
        TextView content = ViewHolder.get(convertView, jiguang.chat.R.id.msg_item_content);
        TextView datetime = ViewHolder.get(convertView, jiguang.chat.R.id.msg_item_date);
        TextView newGroupMsgNumber = ViewHolder.get(convertView, jiguang.chat.R.id.new_group_msg_number);
        TextView newMsgNumber = ViewHolder.get(convertView, jiguang.chat.R.id.new_msg_number);
        ImageView groupBlocked = ViewHolder.get(convertView, jiguang.chat.R.id.iv_groupBlocked);
        ImageView newMsgDisturb = ViewHolder.get(convertView, jiguang.chat.R.id.new_group_msg_disturb);
        ImageView newGroupMsgDisturb = ViewHolder.get(convertView, jiguang.chat.R.id.new_msg_disturb);

        final SwipeLayoutConv swipeLayout = ViewHolder.get(convertView, jiguang.chat.R.id.swp_layout);
        final TextView delete = ViewHolder.get(convertView, jiguang.chat.R.id.tv_delete);

        String draft = mDraftMap.get(convItem.getId());
        if (!TextUtils.isEmpty(convItem.getExtra())) {
            swipeLayout.setBackgroundColor(mContext.getResources().getColor(jiguang.chat.R.color.conv_list_background));
        } else {
            swipeLayout.setBackgroundColor(mContext.getResources().getColor(jiguang.chat.R.color.white));
        }
...
//                //会话界面时间
                ...
                switch (lastMsg.getContentType()) {
                    case image:
                        contentStr = mContext.getString(jiguang.chat.R.string.type_picture);
                        break;
                    case voice:
                        contentStr = mContext.getString(jiguang.chat.R.string.type_voice);
                        break;
                    case location:
                        contentStr = mContext.getString(jiguang.chat.R.string.type_location);
                        break;
                    case file:
                        String extra = lastMsg.getContent().getStringExtra("video");
                        if (!TextUtils.isEmpty(extra)) {
                            contentStr = mContext.getString(jiguang.chat.R.string.type_smallvideo);
                        } else {
                            contentStr = mContext.getString(jiguang.chat.R.string.type_file);
                        }
                        break;
                    case video:
                        contentStr = mContext.getString(jiguang.chat.R.string.type_video);
                        break;
                    case eventNotification:
                        contentStr = mContext.getString(jiguang.chat.R.string.group_notification);
                        break;
                    case custom:
                        ...
                        if (isBlackListHint != null && isBlackListHint) {
                            contentStr = mContext.getString(jiguang.chat.R.string.jmui_server_803008);
                        } else {
                            contentStr = mContext.getString(jiguang.chat.R.string.type_custom);
                        }
                        break;
                    case prompt:
                ...
                if (mAtAll.get(position) && JGApplication.isAtall.get(gid) != null && JGApplication.isAtall.get(gid)) {
                    ...
                    //有人@我 文字提示
                    contentStr = mContext.getString(jiguang.chat.R.string.somebody_at_me) + contentStr;
                    SpannableStringBuilder builder = new SpannableStringBuilder(contentStr);
                    builder.setSpan(new ForegroundColorSpan(Color.RED), 0, 6, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                    content.setText(builder);
                } else {
...
////                                    SpannableStringBuilder builder = new SpannableStringBuilder(contentStr);
////                                    builder.setSpan(new ForegroundColorSpan(mContext.getResources().getColor(jiguang.chat.R.color.line_normal)),
...
//                                }
            ...
            draft = mContext.getString(jiguang.chat.R.string.draft) + draft;
                ...
                mUserInfo.getAvatarBitmap(new GetAvatarBitmapCallback() {
                    ...
                    public void gotResult(int status, String desc, Bitmap bitmap) {
                        if (status == 0) {
                            ...
                            headIcon.setImageResource(jiguang.chat.R.drawable.jmui_head_icon);
                        }
                    }
                });
            } else {
                ...
                mGroupInfo.getAvatarBitmap(new GetAvatarBitmapCallback() {
                    ...
                    public void gotResult(int i, String s, Bitmap bitmap) {
                        if (i == 0) {
                            ...
                            headIcon.setImageResource(jiguang.chat.R.drawable.group);
                        }
                    }
                });
